...
class Fom:
    ...
    def fomCreatNewop(self, line):
        ...
        for operator_dict in new_mutation_trick:
            for operator_father, operator_child_list in operator_dict.items():
                if operator_father == 'delet':
                    fom_messages.append([operator_father, '', 0])
                    continue
    ...
    def mutation_build(self, program_path, mutation_path, mutation_message):
        try:
            lines = File().read_line(program_path)
            for i, om_i in enumerate(mutation_message):
                if not om_i[3] == 'delet':
                    continue
                for j, om_j in enumerate(mutation_message):
                    if j == i:
                        continue
                    if om_j[0] == om_i[0]:
                        print('err：与del生成在同一行')
                        return False
